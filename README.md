...
- **Local LLM cleanup via Ollama** — the raw transcript is post-processed by a
  local model (default `qwen3:8b`) in one of two switchable modes:
...
- **Live pill overlay (v0.2)** — while you hold the key, a small window shows a
  voice scope and a self-correcting interim transcript. The scope is a
  **Nebula** bundle ported from DeadEye's node map — smooth blood-red strands
  that fade in on press, fan wide and ripple with traveling waves while you
  speak, settle narrow when you pause (driven by loudness, not the literal
  waveform), and retract on release. The interim text is a preview only and is
  never injected; the authoritative decode happens on key-up. (Scope on all
  engines; live text is GPU-only.)
...
- **System tray + settings** — mode toggle, ASR engine/model, Ollama model+URL,
  mic device, dictionary editor, and three nebula scope dials (fan sensitivity,
  wiggle, wiggle speed). Settings apply live (the sidecar is hot-reconfigured);
  no restart needed except for a hotkey change.
...
DeadAir.App.exe (C#/.NET 8 WPF)            asr_sidecar (Python)
  ...
  qwen3:8b, transcript cleanup            whisper-server, model warm in VRAM
```
...
- **Ollama** running locally (spec recommends ≥ 0.12.11) with the cleanup model
  pulled: `ollama pull qwen3:8b`. An existing config.json keeps its stored model until changed in Settings (no auto-migration).
...
| `ollama` | `url` / `model` | `http://127.0.0.1:11434` / `qwen3:8b` | `127.0.0.1`, not `localhost` (avoids a Winsock IPv6-first retry stall). |
