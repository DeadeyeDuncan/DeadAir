...
- Segment counts: strands `NebulaSegs = 16`, haze `HazeSegs = 8` (DeadEye's `_segN`/`_segN/2`). *(Superseded by the smoke-tuning loop + T12–T14: final 48/16, turbulence octave + scroll added, dials replace the EnergyGain/TurbSpan consts, SpreadFloor 5.5/Span 7.5, fade-in ignition. The task sections below record each change.)*
