...
| Palette | **Silvered amber** | Same warm family as Lantern; nebula silvering formula over the Lantern lit color. |
...
|---|---|---|
| Strand color (1,2) | `#EEDAC2` = `mix(#FFC77F, #E2E6EE, 0.6)`, 0.85 px, α 0.65 | nebula `_wCol` silvering formula over the Lantern `colLit`; `_wA3` with eLit=1 |
| Hot core (strand 0) | `#FFF9F0`, 1.1 px, α 1.0 | `_hot3` strand: `#fff9f0`, width 1.1 when bright, alpha ×1.9 (clamped) |
| Haze understroke | `#EEDAC2`, 9 px, α 0.05, noise amp ×0.7 | haze pass: 9px, `(0.016+0.034·eLit)` ≈ 0.05, amp ×0.7 |
...
- **XAML:** four new Polylines inside `ScopeCanvas`, z-order bottom→top:
  `HazeLine` (9 px, `#EEDAC2`, α .05) → `Strand1`/`Strand2` (0.85 px,
  `#EEDAC2`, α .65) → `StrandHot` (1.1 px, `#FFF9F0`, α 1.0). Pip shared.
